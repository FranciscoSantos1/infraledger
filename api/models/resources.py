...
class Resource(db.Model):
    ...
    type = db.Column(
        db.Enum(
            "ec2", "rds", "s3", "vm", "k8s_node", "load_balancer", name="resource_type"
        ),
        nullable=False,
    )
    ...
    environment_id = db.Column(
        db.String(36), db.ForeignKey("environments.id"), nullable=False
    )
    ...
    updated_at = db.Column(
        db.DateTime, server_default=db.func.now(), onupdate=lambda: datetime.now(timezone.utc)
    )
