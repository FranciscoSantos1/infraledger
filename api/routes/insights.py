    ...
    return jsonify(
        {
            "resources": [
                {
                    **_serialize_resource(e.resource),
                    "estimated_monthly_cost": float(e.estimated_monthly_cost),
                }
                for e in entries
            ]
        }
...
def dashboard():
    ...
    for entry in latest_entries:
        ...
        cost_by_team[team_name] = cost_by_team.get(team_name, 0) + float(
            entry.estimated_monthly_cost
        )
        cost_by_environment[env_name] = cost_by_environment.get(env_name, 0) + float(
            entry.estimated_monthly_cost
        )

        top_expensive = sorted(
            latest_entries, key=lambda e: e.estimated_monthly_cost, reverse=True
        )[:5]
